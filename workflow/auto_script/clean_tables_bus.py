...
def clean_all_tables_business(schema_name='business_staging', table_suffix='_cleaned',**kwargs):
    """
    ...
    for table_name in tables_to_process:
        try:
            ...
            if table_name == 'product_list':
            
                for chunk in chunk_iter:
                        df_p = drop_index_column(chunk)
                        df_p = remove_duplicates(df_p)
                        df_p = clean_product_type_column(df_p)
                        df_p = handle_missing_values(df_p)
                        df_p = convert_columns_to_uppercase(df_p)

                        cleaned_chunks.append(df_p)
            else:
